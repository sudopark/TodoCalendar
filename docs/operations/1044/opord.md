...
- FRAGO-1 (2026-09-06, 1차 하네스 리뷰 반영) — 부록 A Task 6 Step 1 변경: 종결보고 8항(하네스 갭)은 4·5·9 그룹에서 분리, 사이즈 무관 rules 예약 레코드로 (정본 충돌 report-debrief.md 우선, 유저 확정). campaign 평가 모드는 종결보고 접수(PR 생성) 1회 — 머지는 원장·국면 확정만. 나머지 변경 없음.
- FRAGO-2 (2026-09-06, 2차 하네스 리뷰 반영) — 3-라 인터페이스 계약 추가: 의존 DP 의 stacked 는 유저 명시 허용 시만, 게이트 "선행 DP 머지"를 "선행 PR 존재 + 인터페이스 계약 확정"으로 완화 (orchestrate §2 ↔ opord §3-2 짝, a안 유저 확정). opord §2 의 "campaign 전 M 경로만 실효" 게이트 삭제. 부록 B 재편(사후보고): CLAUDE.md 행은 campaign 커밋에, implement 는 opord 진행 층 커밋에 합류. 나머지 변경 없음.
