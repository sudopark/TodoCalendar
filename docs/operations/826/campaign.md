...
작전계획 — #826 e2e 하네스 구성        작성: 유저   개정: 5회 (재가 전 결심 3건 / DP-1.1 에 host 종단 편입 / 확장 프로세스 격리 DP-1.3 신설 / DP-1.4 를 DP-1.3 에 흡수 / DP-1.3 종결보고 반영)
...
- DP-1.2 → DP-1.3: DP-1.2 가 만든 **스텁 서버 미등록 요청 로그**와 `E2ETestCase`·`E2EFixture` 가 DP-1.3 의 기반이다. DP-1.3 은 시나리오가 쓰던 등록을 베이스로 올려 로그 노이즈를 걷은 뒤, 그 로그와 실 DB mtime 두 축으로 격리를 판정한다. **단 로그 축은 확장이 스텁 host 를 보게 만든 뒤에야 성립한다** — 확장이 실 host 로 나가면 애초에 로그에 안 잡힌다. DP-1.3 이 마커에 스텁 host 를 실어 이 종속을 풀었다.
...
| A-7 | 확장 프로세스에 e2e 실행을 알릴 수단이 존재하고, 그 수단이 일반 실행을 오염시키지 않게 정리할 수 있다 | DP-1.3 설계·실측 | **확인** (DP-1.3) — App Group 컨테이너 마커 파일이 그 수단이다. 오염 주체는 위젯 확장으로 특정됐고, 마커 도입 후 e2e 단독 연속 2회에서 실 저장소 mtime 이 전건 불변이다. 일반 실행 오염은 만료 시각과 일반 실행 시 삭제 두 겹으로 막고 양방향 실측했다. 단 **러너가 쓰는 경로는 불성립** — uiTests 러너 앱의 서명 entitlements 가 비어 공유 컨테이너에 닿지 못한다 | 확장 격리를 포기하고 12항 수용 위험으로 되돌린다 — 그때 최종상태 4항 검증 관점을 앱 프로세스 기준으로 개정 |
...
| 앱이 뜨기 전 구간에 확장이 실 저장소를 연다 | LOE-1 | 수용 — 마커를 쓰는 주체가 앱이라 그 이전 구간은 못 막는다. 연속 실행에서는 앞 실행 마커가 만료(10분) 전까지 남아 창이 안 열리고, 창이 열리는 것은 첫 실행이나 10분 이상 유휴 뒤뿐이다 | 러너가 마커를 쓰는 경로가 불성립이라(A-7) 앱보다 앞서는 주체를 둘 수 없다 |
| 앱 유닛테스트 스킴 실행 중에는 확장이 실 DB 를 연다 | LOE-1 | 수용 — 확장 프로세스엔 `XCTestConfigurationFilePath` 가 없고 마커는 e2e 전용이다. 4항 최종상태는 e2e 실행만 규정한다 | 별도 이슈로 다룰 값이 있으면 캠페인 밖에서 |
