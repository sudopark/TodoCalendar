...
| 2. 매듭 풀기 | 표시·이벤트 모델 5파일이 `CalendarPresentation` 에 있고, 전 위젯 순수 뷰가 `WidgetScenes` 에 있으며, 확장 의존에서 `CalendarScenes` 가 빠졌다 | LOE-1 | 갤러리가 전 위젯을 그릴 수 있게 만든다 |
...
- 순수 뷰 생성자는 최종적으로 `init(model:style:)` 하나로 맞춘다. 엔트리 뷰든 갤러리 미리보기든 같은 걸 부른다. **DP-5.1 전까지는 `init(model:)` 이다** — `WidgetStyle` 이 DP-5.1 산출물이라 미리 만들면 소비자 없는 간접층이 된다 (DP-1.1 확인보고에서 재가). 다만 `EventListView`·`SystemSizeForemostEventView`·`TodayAndNextWidgetView` 셋은 예외다 — 할일 체크박스가 확장 전용 DI 를 쓰는 `TodoToggleIntent` 를 만들어서 그 자리를 클로저로 주입받는다 (DP-2.2, PR #1068 FRAGO-2). DP-5.1 은 이 셋의 생성자에 파라미터가 이미 둘이라는 전제로 간다.
...
- **레지스트리 항목은 노출 조건을 가질 수 있다.** D-day 는 `FeatureFlag.ddayWidget` 으로 가려 둔다 — 지금 번들 등록이 주석이라 갤러리에만 보이면 목록에서 보고도 홈에 못 놓는 위젯이 생긴다. DP-6.1 이 플래그를 켜면 시스템 갤러리·일정 상세 후보 메뉴·앱 갤러리가 한꺼번에 열린다 (2026-09-09 결심).
...
| DP-2.2 | LOE-1 | 2 | D-day 를 뺀 전 위젯군의 순수 뷰를 `WidgetScenes` 로 옮긴다 — 단일 위젯군 AICommand·Today·Foremost·NextEvent·NextRemain·TodayAndNext 와 복합 위젯군 Month·WeekEvents·EventList·Composed 4종을 함께. C1 의 쪼개는 자리 둘을 위젯군마다 그대로 적용한다. 확장의 `CalendarScenes` 의존은 DP-2.1 이 이미 뗐다. **복합 4종은 ViewModel 만 내려갔고 합성 레이아웃은 확장에 남았다** — 분류가 못 걸렀다 (A11, DP-4.1 이 회수한다) | DP-2.1 | `Presentations/WidgetScenes/**`, `Widget/Sources/Widgets/**`(D-day 제외), `Widget/Sources/{Usecases,Base+Factory,Intents}`, `Widget/Snapshots/`, `Widget/Tests/**` | M |
| DP-3.1 | LOE-2 | 3 | 타임라인 위젯. 시간 축을 산출하고(현재~자정, 남은 시간이 4시간 미만이면 12시간으로 연장) 일정을 **가로 막대**로 얹고 겹치면 **레인**을 나눈다. **축은 가로다** — medium 은 밴드 하나에 하루를 깔고 하단 범례가 이름을 받고, large 는 같은 밴드를 얇게 깐 뒤 아래를 일정 목록으로 채운다 (2026-09-09 와꾸 확정, 시안 대조). `EventTypeSelectIntent` 연결까지. 순수 뷰는 처음부터 `WidgetScenes` 에 만들고 ViewModel 의 `.sample` 도 함께 낸다 (C6) | DP-2.2 | `Presentations/WidgetScenes/**`, `Widget/Sources/Widgets/TimelineWidget/**`(신규), `Widget/Sources/Base+Factory/`, `TodoCalendarWidgetBundle.swift`, `Widget/Tests/ViewModelProviders/`, `Supports/Extensions/Resources/*.lproj`, **셀 VM 매핑 중복 정리분** — `Widget/Sources/Widgets/{EventListWidget,TodayAndNext,NextEventWidget}/` 의 provider 3파일 | M |
| DP-4.1 | LOE-3 (LOE-1 겸) | 4 | 갤러리 Scene 을 `WidgetScenes` 안에 세운다. `Scenes+WidgetGallery.swift` 프로토콜, 전 위젯 목록, `.sample` ViewModel 을 넣은 실뷰 미리보기, 설정 > 외관 > 위젯 진입, 앱 루트 조립까지. Composed 4종과 AICommand 의 `.sample` 을 새로 만든다. 복합 4종의 합성 레이아웃도 `WidgetScenes` 로 내린다 (DP-2.2 잔여, A11). 목록은 C6 레지스트리로 세우고, **타임라인 위젯 등재는 DP-3.1 이 머지된 뒤 rebase 해서 얹는다** | DP-2.2 (DP-3.1 과 병행, 머지는 그 뒤) | `Presentations/WidgetScenes/Sources/Gallery/**`, `.sample` 과 복합 콘텐츠 뷰를 더하는 `WidgetScenes/Sources/{Composed,AICommand}/`, 복합 위젯 엔트리 뷰 `Widget/Sources/Widgets/ComposedWidget/**` 4파일, `Presentations/Scenes/Sources/Scenes+WidgetGallery.swift`, `SettingScene` 의 `WidgetAppearanceSettingView` (진입 라우팅 + 가짜 샘플 카드 제거), `TodoCalendarApp/Sources/Root/`, `Supports/Extensions/Resources/*.lproj` | M |
| DP-5.1 | LOE-3 | 5 | C3 의 프리셋 층을 만든다. `WidgetStyle`·`WidgetStylePreset` 과 `resolved` 합성, 위젯 종류별 저장·조회 usecase, environment 전파와 공통 modifier, 확장 진입에서 값 주입, 갤러리 프리셋 선택 UI — 갤러리 2뎁스 변형 페이저는 DP-4.1 이 이미 세웠다(PR #1072 FRAGO-1). DP-5.1 은 그 페이저 위에 프리셋 UI 를 얹는다 | DP-4.1 | `Domain/Sources/Models/Settings/`, `Domain/Sources/Usecases/`, `Repository`(App Group 저장), `Presentations/WidgetScenes/**`, `Widget/Sources/Widgets/**` 진입, `Domain/Tests` | M |
...
| ID | 가정 | 검증 방법 | 깨지면 |
|---|---|---|---|
| A1 | 순수 뷰를 `WidgetScenes` 로 옮겨도 확장이 그리는 모양이 안 바뀐다. C2 대로 배경 그리는 자리를 옮기는 것까지 포함해서다 | **DP-1.1 에서 확인됨** (2026-09-08, PR #1058 — D-day 5패밀리 png 무변화). DP-2.2 에서도 확인됐다 (2026-09-09, PR #1068 — 카탈로그 20장이 develop 기준선과 바이트 동일). **대조 방법을 지정한다** — 스냅샷 스위트는 `withSnapshotTesting(record: .all)` 이라 비교를 안 하고 매번 다시 쓴다. 스위트 통과는 동일의 증거가 아니다. 기준 브랜치를 임시로 체크아웃해 같은 스위트를 돌리고, 거기서 나온 png 와 작업 브랜치 png 를 바이트 비교해야 판정이 선다 (DP-2.1 에서 이 방법으로 18/18 동일 확인, 2026-09-09) | 배경은 확장에 남기고 갤러리는 미리보기용 배경을 따로 그린다. 중심이 약해지므로 즉시보고한다 |
...
| A5 | #751 플랜(`docs/superpowers/plans/2026-08-04-751-timeline-widget.md`)의 축 산출·겹침 배치 부분이 지금 develop 에서 유효하다 | **부분 확인·부분 파기** (2026-09-09, DP-3.1 정찰). 축 산출(현재~자정 + 4시간 미만이면 12시간 연장)은 유효하다. **겹침 배치는 무효다** — 그 플랜은 세로 축을 전제로 열을 나눴는데 와꾸가 가로로 확정돼 레인 배정으로 바뀐다. **Task 0-A 도 무효다** — 셀 VM 매핑은 DP-2.1 이 `EventCellViewModelMapper` 로 이미 내려놔서 추출이 아니라 소비다 | opord 로 새로 쓴다. **뷰 배치 부분은 이미 무효다** — 그 플랜은 순수 뷰를 확장에 두는 전제로 쓰였고 DP-1.1 이 전제를 바꾼다 |
...
| A7 | 확장이 `WidgetScenes` 를 물어도 위젯 확장 메모리 한도(약 30MB)에 안 걸린다. `CalendarScenes` 를 떼는 만큼 상쇄된다 | DP-1.1 에서 실기로 배치하고, DP-4.1 에서 갤러리가 들어간 뒤 다시 본다. **DP-4.1 결과** (2026-09-09, PR #1072): 확장 빌드 통과·PIR-1 낌새 없음, 메모리 실측은 미수행 — `WidgetScenes` 를 더 키우는 DP-5.x 에서 실측한다 | 갤러리 Scene 을 별도 모듈로 떼고 `WidgetScenes` 엔 뷰만 남긴다 |
| A8 | `SettingScene` → `Scenes` 프로토콜 → `WidgetScenes` 배선이 기존 Scene 간 참조 패턴대로 된다 | **DP-4.1 에서 확인됨** (2026-09-09, PR #1072 — 형제 `SettingItemList*` 와 같은 구성으로 성립) | 갤러리를 `SettingScene` 안에 두고 뷰만 `WidgetScenes` 에서 가져다 쓴다 |
| A9 | 신규 위젯을 무료로 풀어도 Pro 가치가 안 죽는다. Pro 가 파는 건 전 위젯 꾸미기 깊이다 | 출시 후 지표로 본다 | 위젯 자체를 잠그는 쪽으로 되돌린다. 계획을 개정한다 |
| A12 | 세로 시간 축이 위젯 캔버스에 들어간다 | **깨졌다** (2026-09-09, DP-3.1 와꾸 시안). medium 가용 높이가 138px 인데 축이 현재~자정이면 아침엔 16시간이라 8.6px/h 다 — 1시간 일정이 8.6px, 30분이 4px 라 색 막대만 남는다. 플랜의 "4시간 미만이면 12시간 연장" 은 축이 **짧을** 때만 다루고 실제 파탄인 **긴** 쪽을 안 건드린다 | **가로 축으로 간다.** 폭 294px 에 하루를 깔면 시각과 무관하게 밀도가 일정하고, medium 은 범례가 large 는 목록이 이름을 받는다. 세로 축으로 되돌아갈 유일한 경로는 축을 고정 창(예: 6시간)으로 자르는 것인데, 그러면 "현재~자정" 이 깨져 계획 개정이 더 커진다 |
| A11 | 순수 뷰와 엔트리 뷰를 `model:` 을 받나 `entry:` 를 받나로 가르면 전 위젯이 갈린다 | **DP-4.1 정찰에서 깨졌다** (2026-09-09, #1069). 복합 위젯 4종은 엔트리 뷰가 `entry` 를 풀면서 합성 레이아웃(`HStack { EventListView + SystemSizeForemostEventView }` — `EventAndForemostWidget.swift:36`)까지 겸해서, DP-2.2 의 구조적 분류에 안 걸리고 확장에 남았다. 그 결과 `WidgetScenes/Sources/Composed/` 엔 ViewModel 만 있고 뷰가 없다 | **깨졌다 — 해소 완료** (2026-09-09, PR #1072 T-2, 기준선 png 4장 바이트 동일). 복합 콘텐츠 뷰 4개를 `ComposedWidgetViews.swift` 로 뽑아 DP-4.1 에서 함께 내린다. 안 그러면 갤러리 20종 중 4종만 미리보기와 실물이 다른 뷰가 되어 11항 LOE-1 MOE 가 그 넷에서 깨진다. 옮기기만 하고 모양은 안 바꾸며, 카탈로그의 `widget-event-and-foremost`·`widget-event-and-month` 두 장이 바이트 대조 그물이다 |
| A10 | 순수 뷰가 `AppIntents` 를 물어도 갤러리가 선다. C1 이 막은 것은 WidgetKit·`CalendarScenes`·`Scenes` 셋이고 `AppIntents` 는 그 목록에 없다 | DP-2.2 에서는 물지 않고 피했다 — 할일 토글이 확장 전용 DI 를 쓰는 intent 를 만들어서, 그 자리를 순수 뷰가 클로저로 주입받게 했다 (PR #1068 FRAGO-2). DP-4.1 이 갤러리에서 인터랙티브 위젯을 미리보기로 그릴 때 같은 문제가 다시 온다. **DP-4.1 결과** (2026-09-09, PR #1072): 미리보기를 표시 전용 체크박스로 잘라 성립 — 대응 칸의 표시용 절단 경로를 탔다 | 갤러리 미리보기는 인터랙션 없는 표시용으로 자르고, 그 사실을 유저에게 알린다 |
...
| DP-2.2 가 위젯군 9개·33파일을 한 PR 에 담아 리뷰 단위를 넘긴다 | LOE-1 | **해소** — 커밋을 위젯군 단위로 끊어 PR #1068 이 커밋 11개로 나뉘었다. 후속 DP 분리는 필요하지 않았다 | 유저 지시로 DP-2.2·DP-2.3 을 병합했다 (2026-09-09). 대상이 약 5,900 줄이었다 |
...
| 갤러리 레지스트리 커버리지 테스트가 확장 `Widget.kind` 20개를 사본으로 든다 — 확장에 위젯이 늘면 사본을 손으로 맞춰야 한다 | LOE-3 | 수용 — 커버리지 단언(`registry_variantKindsCoverAllPlaceableWidgetDeclarations`)이 등재 누락을 잡고, 사본 갱신은 그 테스트의 실패가 지시한다 | PR #1072 리뷰 지적 (2026-09-09) |
...
- LOE-3 — 갤러리 목록이 **홈·잠금화면에 놓을 수 있는 위젯 수**와 맞나. `Widget` 선언 21개에서 `EventCountdownLiveActivity` 를 뺀 20개가 기준이고, 라이브 액티비티와 iOS 18 `AICommandControlWidget` 은 유저가 위젯으로 배치하는 물건이 아니라 셈 밖이다. 갤러리에서 바꾼 값이 홈 위젯에 반영되나. 프리셋과 조정이 `requiresPro` 로 갈리나.
