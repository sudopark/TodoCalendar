...
| A11 | 순수 뷰와 엔트리 뷰를 `model:` 을 받나 `entry:` 를 받나로 가르면 전 위젯이 갈린다 | **DP-4.1 정찰에서 깨졌다** (2026-09-09, #1069). 복합 위젯 4종은 엔트리 뷰가 `entry` 를 풀면서 합성 레이아웃(`HStack { EventListView + SystemSizeForemostEventView }` — `EventAndForemostWidget.swift:36`)까지 겸해서, DP-2.2 의 구조적 분류에 안 걸리고 확장에 남았다. 그 결과 `WidgetScenes/Sources/Composed/` 엔 ViewModel 만 있고 뷰가 없다 | **깨졌다 — 대응 실행 중.** 복합 콘텐츠 뷰 4개를 `ComposedWidgetViews.swift` 로 뽑아 DP-4.1 에서 함께 내린다. 안 그러면 갤러리 20종 중 4종만 미리보기와 실물이 다른 뷰가 되어 11항 LOE-1 MOE 가 그 넷에서 깨진다. 옮기기만 하고 모양은 안 바꾸며, 카탈로그의 `widget-event-and-foremost`·`widget-event-and-month` 두 장이 바이트 대조 그물이다 |
| A10 | 순수 뷰가 `AppIntents` 를 물어도 갤러리가 선다. C1 이 막은 것은 WidgetKit·`CalendarScenes`·`Scenes` 셋이고 `AppIntents` 는 그 목록에 없다 | DP-2.2 에서는 물지 않고 피했다 — 할일 토글이 확장 전용 DI 를 쓰는 intent 를 만들어서, 그 자리를 순수 뷰가 클로저로 주입받게 했다 (PR #1068 FRAGO-2). DP-4.1 이 갤러리에서 인터랙티브 위젯을 미리보기로 그릴 때 같은 문제가 다시 온다 | 갤러리 미리보기는 인터랙션 없는 표시용으로 자르고, 그 사실을 유저에게 알린다 |
