//
//  TodayAndMonthStyleSelectIntentTests.swift
...
struct TodayAndMonthStyleEntityListTests {

    private func style(
        _ style: WidgetStyleId.Style, name: String?
    ) -> WidgetStyle {
        return .init(
            id: .init(variant: .todayAndMonthMedium, style: style),
            name: name,
    ...
    func styleEntities_keepListOrderWithDefaultFirst() {
        ...
        // when
        let entities = styles.map { TodayAndMonthStyleEntity(style: $0) }
...
struct TodayAndMonthWidgetConfigurationIntentTests {

    private func intent(selecting entityId: String?) -> TodayAndMonthWidgetConfigurationIntent {
        let intent = TodayAndMonthWidgetConfigurationIntent()
        intent.style = entityId.map { TodayAndMonthStyleEntity(id: $0, name: "any") }
        return intent
    }
