//
//  EventListStyleSelectIntentTests.swift
...
struct EventListStyleEntityListTests {

    private func style(
        _ style: WidgetStyleId.Style, name: String?
    ) -> WidgetStyle {
        return .init(
            id: .init(variant: .eventListSmall, style: style),
            name: name,
            setting: EventListStyleSetting.initial
        )
    }
    ...
    func styleEntities_keepListOrderWithDefaultFirst() {
        ...
        // when
        let entities = styles.map { EventListStyleEntity(style: $0) }
...
struct EventListStyleParameterTests {

    private func intent(selecting entityId: String?) -> EventTypeSelectIntent {
        let intent = EventTypeSelectIntent()
        intent.style = entityId.map { EventListStyleEntity(id: $0, name: "any") }
        return intent
    }
    ...
    func existingParameters_unchangedAfterStyleAdded() {
        ...
        // when
        intent.style = EventListStyleEntity(id: "custom::c1", name: "Night")
