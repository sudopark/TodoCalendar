//
//  DoubleMonthStyleSelectIntentTests.swift
...
struct DoubleMonthStyleEntityListTests {

    private func style(
        _ style: WidgetStyleId.Style, name: String?
    ) -> WidgetStyle {
        return .init(
            id: .init(variant: .doubleMonthMedium, style: style),
            name: name,
            setting: DoubleMonthStyleSetting.initial
        )
    }
    ...
    func styleEntities_keepListOrderWithDefaultFirst() {
        ...
        // when
        let entities = styles.map { DoubleMonthStyleEntity(style: $0) }
...
struct DoubleMonthWidgetConfigurationIntentTests {

    private func intent(selecting entityId: String?) -> DoubleMonthWidgetConfigurationIntent {
        let intent = DoubleMonthWidgetConfigurationIntent()
        intent.style = entityId.map { DoubleMonthStyleEntity(id: $0, name: "any") }
        return intent
    }
