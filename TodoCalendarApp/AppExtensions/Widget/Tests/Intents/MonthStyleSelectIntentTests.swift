//
//  MonthStyleSelectIntentTests.swift
...
struct MonthStyleEntityListTests {

    private func style(
        _ style: WidgetStyleId.Style, name: String?
    ) -> WidgetStyle {
        return .init(
            id: .init(variant: .monthSmall, style: style),
            name: name,
            setting: MonthStyleSetting.initial
        )
    }
    ...
    func styleEntities_keepListOrderWithDefaultFirst() {
        ...
        // when
        let entities = styles.map { MonthStyleEntity(style: $0) }
...
struct MonthWidgetConfigurationIntentTests {

    private func intent(selecting entityId: String?) -> MonthWidgetConfigurationIntent {
        let intent = MonthWidgetConfigurationIntent()
        intent.style = entityId.map { MonthStyleEntity(id: $0, name: "any") }
        return intent
    }
