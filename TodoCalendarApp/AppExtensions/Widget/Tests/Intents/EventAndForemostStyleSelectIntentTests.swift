//
//  EventAndForemostStyleSelectIntentTests.swift
...
struct EventAndForemostStyleEntityListTests {

    private func style(
        _ style: WidgetStyleId.Style, name: String?
    ) -> WidgetStyle {
        return .init(
            id: .init(variant: .eventAndForemostMedium, style: style),
            name: name,
    ...
    func styleEntities_keepListOrderWithDefaultFirst() {
        ...
        // when
        let entities = styles.map { EventAndForemostStyleEntity(style: $0) }
...
struct EventAndForemostWidgetConfigurationIntentTests {

    private func intent(selecting entityId: String?) -> EventAndForemostWidgetConfigurationIntent {
        let intent = EventAndForemostWidgetConfigurationIntent()
        intent.style = entityId.map { EventAndForemostStyleEntity(id: $0, name: "any") }
        return intent
    }
