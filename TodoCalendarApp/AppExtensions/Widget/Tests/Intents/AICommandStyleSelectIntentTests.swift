//
//  AICommandStyleSelectIntentTests.swift
...
struct AICommandStyleEntityListTests {

    private func style(
        _ style: WidgetStyleId.Style, name: String?
    ) -> WidgetStyle {
        return .init(
            id: .init(variant: .aiCommandSmall, style: style),
            name: name,
            setting: AICommandStyleSetting.initial
        )
    }
    ...
    func styleEntities_keepListOrderWithDefaultFirst() {
        ...
        // when
        let entities = styles.map { AICommandStyleEntity(style: $0) }
...
struct AICommandWidgetConfigurationIntentTests {

    private func intent(selecting entityId: String?) -> AICommandWidgetConfigurationIntent {
        let intent = AICommandWidgetConfigurationIntent()
        intent.style = entityId.map { AICommandStyleEntity(id: $0, name: "any") }
        return intent
    }
