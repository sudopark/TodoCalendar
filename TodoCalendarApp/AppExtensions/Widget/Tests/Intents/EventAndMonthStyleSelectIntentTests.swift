//
//  EventAndMonthStyleSelectIntentTests.swift
...
struct EventAndMonthStyleEntityListTests {

    private func style(
        _ style: WidgetStyleId.Style, name: String?
    ) -> WidgetStyle {
        return .init(
            id: .init(variant: .eventAndMonthMedium, style: style),
            name: name,
            setting: EventAndMonthStyleSetting.initial
        )
    }
    ...
    func styleEntities_keepListOrderWithDefaultFirst() {
        ...
        // when
        let entities = styles.map { EventAndMonthStyleEntity(style: $0) }
...
struct EventAndMonthWidgetConfigurationIntentTests {

    private func intent(selecting entityId: String?) -> EventAndMonthWidgetConfigurationIntent {
        let intent = EventAndMonthWidgetConfigurationIntent()
        intent.style = entityId.map { EventAndMonthStyleEntity(id: $0, name: "any") }
        return intent
    }
