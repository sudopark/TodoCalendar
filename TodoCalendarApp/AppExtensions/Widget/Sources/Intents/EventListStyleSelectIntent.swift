//
//  EventListStyleSelectIntent.swift
//  TodoCalendarApp
//
...
// MARK: - EventListStyleEntity

struct EventListStyleEntity: AppEntity, Sendable {

    static let typeDisplayRepresentation: TypeDisplayRepresentation = "WidgetStyle"
    static let defaultQuery = EventListStyleQuery()
...
extension EventListStyleEntity {
...
// MARK: - EventListStyleQuery

/// 조회할 변형을 타입 안에 박으므로 위젯군마다 엔티티·쿼리가 따로 선다.
struct EventListStyleQuery: EntityQuery, @unchecked Sendable {
    ...
    func entities(for identifiers: [String]) async throws -> [EventListStyleEntity] {
        let selectedIds = Set(identifiers)
        return self.savedStyleEntities().filter { selectedIds.contains($0.id) }
    }

    func suggestedEntities() async throws -> [EventListStyleEntity] {
        return self.savedStyleEntities()
    }

    private func savedStyleEntities() -> [EventListStyleEntity] {
        return self.factory.makeStyleUsecase()
            .loadStyles(of: .eventListSmall)
            .map { EventListStyleEntity(style: $0) }
    }
}
