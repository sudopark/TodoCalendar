...
- **RED 직후 축1 선언** — 방금 쓴 TC가 표현하는 명세 항목(위 목록의 한 항목)을 `축1: <TC명> ⇢ <명세 항목>` 한 줄로 선언한다. 관문의 판정 질문은 "TC가 있는가"가 아니라 **"이 단언이 실패하려면 프로덕션의 무엇이 깨져야 하나"**다 — 답이 "아무것도 없음"이면(느슨한 단언·스텁이 못 만드는 경로·도메인상 불가능한 픽스처) 그 TC는 없는 것과 같다. 명세를 못 담는 TC면 GREEN으로 가지 않고 TC부터 재작성한다. 선언 시 **이 TC**와 **이 명세 항목의 TC 집합**을 각각 확인한다:
  ...
  *이 명세 항목의 TC 집합* —
  ...
  - **형제 기준선** — 이번에 추가·수정한 메서드의 형제(같은 타입·같은 프로토콜의 다른 메서드)가 가진 TC 종류를 grep해 대조한다. 형제가 실패 케이스 3종을 갖는데 새 메서드엔 성공 경로만 붙었으면, 그 차이에 근거가 있나. 신규 대상만 자기완결적으로 보면 바로 옆의 커버리지 기준선이 시야에 안 들어온다
...
- **신설물 rules 대조** — 이번 diff가 **새로 만든** 타입·컴포넌트·상수 묶음·파일 각각에 대해, 그 종류를 규정하는 rules 조항을 다시 연다. 착수 시점 rules 확인은 "무엇을 만들지" 모르는 상태의 경로 기반 확인이라 신설물을 안 덮는다 — 이름·배치·카탈로그 등재 의무는 물건이 생긴 뒤에만 판정된다
