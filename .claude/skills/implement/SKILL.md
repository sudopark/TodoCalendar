...
- 서브에이전트에 구현을 dispatch할 땐 브리프에 적용 rules 조항의 **요지를 발췌해 싣는다** — 파일 경로·조항명만 던지지 않는다 (path 매칭 자동 로드는 서브에이전트에겐 없다). 검증 범위(완료 판정의 사다리로 산정한 최소 실행 범위)·따라야 할 구조 패턴도 함께 명시한다 — "전체 스킴 통과"를 기본값으로 승계하지 않는다.
