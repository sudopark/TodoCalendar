...
- **RED 직후 축1 선언** — 방금 쓴 TC가 표현하는 명세 항목(플랜 태스크 성공 기준·이슈 스펙 조항)을 `축1: <TC명> ⇢ <명세 항목>` 한 줄로 선언한다. 명세를 못 담는 TC면 GREEN으로 가지 않고 TC부터 재작성한다. 선언 시 아래 셋을 함께 확인한다:
  ...
  - **판별력** — given을 다르게 골라도 이 TC가 통과하나. 통과하면 시스템 관점 델타가 0이라 시나리오를 다시 고른다
