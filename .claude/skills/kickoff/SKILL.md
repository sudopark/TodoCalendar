...
2. 단일 관심사 — 결정거리가 하나로 수렴한다. 같은 변경을 여러 파일에 기계적으로 반복하는 치환이면 파일 수는 따지지 않는다
3. TDD 사이클 0~1회 (문서·설정·리소스 변경 포함)
4. 서브에이전트 dispatch·병렬 실행 없음

하나라도 어긋나면 plan 스킬로 간다. 생략할 땐 A-2 스펙 브리프에 실행 항목 목록을 담아 A-4를 갈음한다 — 판단 근거는 트랙 판정 선언에 한 줄로 밝힌다.
