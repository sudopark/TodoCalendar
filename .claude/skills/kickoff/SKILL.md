...
**전제 붕괴 종료** — 게이트·브리프 확인 과정에서 이슈의 전제가 무너지면(요구가 이미 해소됨·성립하지 않음) 브리프도 플랜도 만들지 않는다. 무너진 전제와 근거를 유저에게 보여주고 확인받은 뒤 이슈를 닫고(`gh issue close --comment`) 킥오프를 종료한다. 이 종료 코멘트는 위 코멘트 상한의 예외다 — 브리프 자리가 없어진 경로라 사유를 남길 데가 여기뿐이다.
