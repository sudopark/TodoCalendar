...
**생략 조건** — 이슈 본문이 이미 완료 기준·스코프·현재 동작을 담고 있고, 게이트에서 반문으로 확정된 결정이 하나도 없으면(AskUserQuestion 0라운드) 브리프가 본문 재서술이라 게시하지 않는다. 반문으로 확정된 게 하나라도 있으면 필수 — 그 답은 이슈 어디에도 없어 다음 세션이 복원할 방법이 없다.
