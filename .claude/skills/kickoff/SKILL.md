...
**코드베이스** — Explore 서브에이전트로 fan-out (독립 영역은 병렬로). 각 프롬프트에 반드시 포함:
- (a) 변경 대상의 **현재 동작을 file:line 근거로** 보고할 것
- (b) 동종 컴포넌트의 **구조 패턴**(상태관리·합성·추상화 수준)을 파악해 보고할 것
- (c) 해당 경로에 적용되는 `.claude/rules/*.md` 조항을 확인할 것
