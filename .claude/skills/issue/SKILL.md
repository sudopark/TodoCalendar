...
- 하위 이슈 본문은 목표 한두 줄 + 상위 좌표 (`상위 이슈: #N / DP-<x.y>` — 작전계획 campaign.md 7항의 DP 좌표. 계획 없는 단순 하위면 `상위 이슈: #N`) — 디테일은 그 이슈의 킥오프 게이트에서 채워진다. DP 이슈 생성 시 campaign 원장의 이슈# 칸을 채우고 상태를 `착수` 로 올린 뒤, 상위 이슈 본문 미러를 재조립한다(campaign §5).
