...
- **컨벤션·rules 정합** — rules 조항, CLAUDE.md 짝지어진 두 위치, 그리고 **문서화되지 않고 코드에만 존재하는 관례**(형제 컴포넌트의 타입 형태·의존 주입 방식·네이밍). rules에 없다고 관례가 아닌 게 아니다 — **이번 diff가 새로 만든 파일·타입·서비스·팩토리, 그리고 새로 고른 배치 형태**가 들어오면 형제를 연다. 파일은 새 타입을 안 담아도 신설물이다 (implement 리팩터 게이트의 신설물 범위와 같은 폭). **선례 없는 배치를 골랐으면 그 대가를 한 줄로 답한다** — 가시성이 넓어졌나, 짝지어 갱신할 자리가 늘었나. PR #1143 에서 구현체를 기능축으로 쪼갠 파일이 형제 22개가 전부 `<모델>+Mapping` 계열인 자리에 끼어들었고, 파일 스코프 `private` 12개가 `internal` 로 서는 대가가 리뷰에서 안 보였다
- **문서-실물 정합** — 스킬·docs·주석·커밋 메시지가 실물 코드와 일치하는가
- **테스트 적정성** — testability 규칙 준수, 커버리지 공백, 테스트가 실동작을 검증하는가. **판별력은 implement §구현 중의 축1 조항(판별력·픽스처 유효성·더블 왕복)을 리뷰어가 독립으로 다시 돌린다** — 그쪽은 작성 세션의 자기선언이라 선언한 세션이 못 본 것을 못 잡는다. 관문이 두 겹이 되려면 같은 축을 여기서 다시 봐야 한다. 항목을 여기 복제하지 않는다 — 복제하면 개정 때 한쪽이 조용히 stale 해진다
