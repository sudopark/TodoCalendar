...
- 입력: **M** 이면 kickoff 스펙 브리프. **L 의 DP** 면 DP ID + `campaign.md` 인용(1-다).
...
6. **확인보고 → 재가** — 서두 `■ 확인보고`(`report-confirmation.md`: 임무 내 말로 / 의도 / 자율로 정할 것 / 묻는 것)를 채워 유저에게 **응답 본문에 전문을** 보인다. 전문은 응답 본문과 **함께** `report-confirmation.md` 머리 게시 줄대로 이슈에 봇 코멘트로 게시한다 (`mcp__github-reviewer__add_issue_comment` + `@sudopark` 멘션 — 재가 대기). 재가에서 유저가 고칠 범위도 템플릿 사용 노트대로. 재가되면 진행 파일의 명령 상태를 `재가` 로 올리고 §7 미러.
...
명령 상태는 진행 파일(§8)의 `명령 상태:` 줄에 산다 — 커밋 층(opord.md)엔 상태 칸이 없다. 값은 넷이고, 바꾸는 주체가 다르다:
...
- 이슈 본문 = 커밋 층 전문 + `<!-- progress -->` 블록. 재가 직후와 진행 갱신마다 재조립한다 — opord.md 와 progress.md 를 이어 붙여 `gh issue edit <N> --body-file <합본>`. 별도 요약 코멘트는 없다(kickoff A-4 갈음). 마커 코멘트도 없다 — 본문 자체가 최신 확정 상태다. 보고 봇 코멘트(확인보고·종결보고 등 게시 줄이 ○인 것)는 이 금지의 대상이 아니다 — 그건 히스토리 층이다(issue 스킬).
