...
- 확인이 안 되면 **그 자리에서 멈춘다.** 미확정 전제 위에 다음 추론을 얹지 않는다 — "아마 그럴 것"으로 이어붙인 체인은 몇 단계만 가도 근거가 남지 않는다. 특히 "이 조항이 저 스킬과 충돌한다"는 판정은 상대 조항 원문을 열어 확인한 뒤에만 성립한다.
- 확인 수단이 없는 전제(실행 시 Claude의 해석·미래 세션 동작)는 finding으로 승격하지 않고 "검증 필요"로만 남긴다.
- finding 본문의 근거 체인은 확인된 file:line만으로 구성한다. 체인에 미확정 고리가 하나라도 있으면 그 finding은 보고하지 않는다.
...
finding 목록 뒤에 **확인하지 못한 것**을 붙인다 — 안 본 영역, 확인 수단이 없어 "검증 필요"로 접은 지점. 없으면 "없음"으로 명시한다. 비워두면 컨트롤러가 "파봤는데 없음"과 "못 파서 없음"을 구분하지 못한다.
