...
- **예외 — `EnvironmentStorage`(App Group UserDefaults) 기반 설정 저장소는 `load` 를 쓴다.** 이 구분은 "원격 호출이 수반되나"를 알리려는 것인데 설정 계열엔 원격 변형이 아예 없어 구분이 할 일이 없다. 기존 예: `AppSettingRepository.loadWidgetAppearanceSetting()`, `DDayCandidateRepository.loadCandidates()`, `WidgetStyleRepository.loadStyle(for:)`.
